...
# But first—an aside: statistical summaries 
...
mobility_covdata |> 
# Isolating data from London:
filter(str_detect(city, "Lon")) |>  
ggplot(aes(x = date, 
           y = score, 
...
penguins |>  # Dropping missing values (for simplicity): 
drop_na() |>  
...
# You'll want to hit zoom if you're reviewing this plot in RStudio!
...
# Use the aus.fert data frame to produce a heatmap of fertility rates in 
# Australia in the 20th century. Can you use geom_tile() as an 
# alternative to geom_raster()?
...
# 15 in 1995—i.e., the 1980 cohort!
